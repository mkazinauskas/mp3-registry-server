...
class CreateJamendoJsonEntriesIntegrationSpec extends Specification {

    @Autowired
    private Entries repository

    @Shared
    private File file = new File('src/test/resources/jamendo/json/sample-jamendo-result-formatted.json')
    ...
    def 'should fail to execute request to api'() {
        when:
            ResponseEntity<String> response = requestTemplate.post(
                    '/register/entries/jamendo-json', [:]
            )
        then:
            response.statusCode == BAD_REQUEST
            def responseJson = new JsonSlurper().parseText(response.body)
            responseJson.path == '/register/entries/jamendo-json'
            Map<String, HashMap> errors = responseJson.errors.collectEntries { [it.field, it] }
            errors.jsonResponse.rejectedValue == null
    }

    def 'should persist jamendo json changes to api'() {
        given:
            def request = [
                    jsonResponse: file.getText('UTF-8')
            ]
        when:
            ResponseEntity<String> response = requestTemplate.post(
                    '/register/entries/jamendo-json',
        ...
        then:
            ...
            entry.value != null
            shouldContainMainElements(entry.value)
            entry.created != null
            entry.checksum != null
            entry.sourceType == SourceType.JAMENDO_JSON
    }

    void shouldContainMainElements(String value) {
        def text = new JsonSlurper().parseText(value)
        assert text.id == '64073'
        assert text.artist_name == 'Dzynek'
        assert text.license_ccurl == 'http://creativecommons.org/licenses/by-sa/3.0/'
        assert text.name == 'Test drive'
        assert text.album_name == 'Adventure'
    }
}